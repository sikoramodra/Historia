<script>
  ...
  const searchProducts = dbData.map((product) => ({
    ...product,
    searchTerms: `${product.name} ${product.other_names
      .toString()
      .replace(/,/g, " ")} ${product.code_names
      .toString()
      .replace(/,/g, " ")} ${
      product.birth_place
    } ${product.birth_date.substring(0, 4)} ${
      product.death_place
    } ${product.death_date.substring(0, 4)} ${product.grave} `,
  }));
    ...
    try {
      ...
      dbData = dbData.map((product) => ({
        ...
        searchTerms: `${product.name} ${product.other_names
          ...
          .replace(/,/g, " ")} ${product.code_names
          .toString()
          .replace(/,/g, " ")} ${product.birth_place} ${
          product.birth_date === null ? "" : product.birth_date.substring(0, 4)
        } ${product.death_place} ${
          product.death_date === null ? "" : product.death_date.substring(0, 4)
        } ${product.grave} `,
      }));
      console.log(dbData);
...
<div
  class="min-h-screen flex flex-col items-center justify-center text-white bg-gradient-to-b from-slate-950 to-slate-800 relative"
>
  ...
  <!-- Search Bar -->
  <div
    class="text-2xl xl:text-3xl font-semibold flex items-center h-1/2 mt-12 relative z-10"
  >
    <span class="absolute left-4 top-1/2 transform -translate-y-1/2">
      ...
      >
        <path
          ...
          stroke-width="2"
          d="M11 2a9 9 0 0 1 9 9c0 5-4 9-9 9a9 9 0 0 1-9-9c0-5 4-9 9-9z"
        />
        <path
          stroke-linecap="round"
          stroke-linejoin="round"
    ...
    >
      {#each $searchStore.filtered as person}
        <div
          ...
          role="button"
          tabindex="0"
          class="bg-red-50 h-min rounded-lg m-1 flex-grow basis-auto border {person
  ...
  {#if !$searchStore.search}
    <Link to="/add-person" class="absolute bottom-4 right-4 user-select-none">
      ...
      >
        ...
        <span class="text-white font-semibold tracking-wide user-select-none"
          >Dodaj Osobę</span
        >
      </div>
    </Link>
